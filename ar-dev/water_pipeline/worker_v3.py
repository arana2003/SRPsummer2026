...
def load_molecule(mol_name):
    if mol_name == 'water':
        # Load PBQFF NC modes + calibrated scales
        d  = os.environ.get('MOLDATA_DIR', '/pscratch/sd/a/alexisr/water_modes')

        geom_ang  = np.load(f'{d}/water_pbqff_geom.npy')
        labels    = list(np.load(f'{d}/water_pbqff_labels.npy'))
        modes_arr = np.load(f'{d}/water_pbqff_NC_modes.npy')   # shape (n_modes, 3*n_atoms)
        freqs_arr = np.load(f'{d}/water_pbqff_NC_freqs.npy')
        scales_arr= np.load(f'{d}/water_pbqff_scales.npy')     # Ang/q per mode
        ...
        return {
            ...
            'scales':    {i+1: scales_arr[i] for i in range(len(freqs_arr))},
        }
...
def precompute(mol, mi, mj):
    ...
    return {
        ...
        'scale_i':   mol['scales'][mi],
        'scale_j':   mol['scales'][mj],
...
def make_geometry(mol, pre, qi, qj):
    ...
    Uses pre-calibrated scale factors (Ang/q) per mode.
    delta_x_ang = q * scale * L_vec
    """
    n = pre['n_atoms']
    vec_i = pre['vec_i'].reshape(n, 3)
    vec_j = pre['vec_j'].reshape(n, 3)
    scale_i = pre['scale_i']  # Ang per unit q
    scale_j = pre['scale_j']

    geom = mol['geom_ang'] + qi*scale_i*vec_i + qj*scale_j*vec_j
